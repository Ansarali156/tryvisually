...
import { Clock, BookOpen, CheckCircle, Info } from "lucide-react";
import { Badge } from "@/components/ui/badge";

export interface LinkedListComplexityCardProps {
  operation: LinkedListOperationType;
  variant: LinkedListVariant;
  className?: string;
}

export function LinkedListComplexityCard({
  operation,
  variant,
  className,
}: LinkedListComplexityCardProps) {
  const opMeta =
    LINKED_LIST_OPERATIONS.find((op) => op.id === operation) ||
  ...
  return (
    ...
    >
      ...
      {/* 2. Educational Section: What is a Linked List? */}
      <div className="space-y-3">
        <div className="flex items-center gap-2">
        ...
        <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed font-sans">
          ...
          memory locations. Each element (node) contains data and one or more reference pointers
          linking to adjacent nodes.
        </p>

        {/* Variant specific notes */}
        <div className="p-2.5 rounded-xl bg-surface-50 dark:bg-surface-950 border border-slate-100 dark:border-slate-800 flex items-start gap-2 text-xs">
          ...
          <div>
            <span className="font-semibold text-slate-800 dark:text-slate-200 block capitalize">
              {variant} Variant Characteristics
            </span>
            <span className="text-slate-500 dark:text-slate-400">
              {variant === "singly" &&
                "Unidirectional traversal. Each node holds a single 'next' pointer. Ends at NULL."}
              {variant === "doubly" &&
                "Bidirectional traversal. Nodes hold both 'prev' and 'next' pointers, allowing O(1) backward steps."}
              {variant === "circular" &&
                "Continuous loop. TAIL.next points back to HEAD instead of NULL. Requires loop detection to terminate."}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-400 font-sans">
          <div className="flex items-start gap-2 p-2 rounded-lg bg-surface-50 dark:bg-surface-950">
            ...
            <div>
              ...
              <span>Inserting or deleting at the head requires only pointer updates, no element shifting.</span>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 p-2 rounded-lg bg-surface-50 dark:bg-surface-950">
            ...
            <div>
              ...
              <span>No random index access. Traversing to index k requires following k pointer hops (O(n)).</span>
            </div>
          </div>
        </div>
      </div>
